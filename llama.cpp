...
static size_t llama_set_vram_budget(double budget_gb, int gpu_device) {
  ...
  if (budget_gb < 0) {
    ...
    vram_budget_bytes =
        ggml_cuda_get_free_memory(gpu_device) - 256 * 1024 * 1024;
  } else {
...
#else
  if (budget_gb < 0) {
    vram_budget_bytes = 10ULL * 1024 * 1024 * 1024; // 10GB default
  } else {
    vram_budget_bytes = (size_t)(budget_gb * 1024 * 1024 * 1024);
  }
#endif
...
// [UMA-FIX BP4a] Original: threw exception when CUDA not defined
// #if not defined(GGML_USE_CUBLAS)
//   throw std::runtime_error("CUDA is not enabled");
// #endif
#if !defined(GGML_USE_CUBLAS) && !defined(GGML_USE_METAL)
  // No GPU backend available, cannot reduce VRAM budget
  (void)budget_bytes;
  return false;
#endif
...
struct LLM_TN {
  ...
  std::pair<std::string, llm_tensor>
  operator()(llm_tensor tensor, const std::string &suffix) const {
    return std::make_pair(LLM_TENSOR_NAMES[arch].at(tensor) + "." + suffix,
                          tensor);
  }

  std::pair<std::string, llm_tensor> operator()(llm_tensor tensor,
                                                int bid) const {
    return std::make_pair(
        ::format(LLM_TENSOR_NAMES[arch].at(tensor).c_str(), bid), tensor);
  }

  std::pair<std::string, llm_tensor>
  operator()(llm_tensor tensor, const std::string &suffix, int bid) const {
    return std::make_pair(
        ::format(LLM_TENSOR_NAMES[arch].at(tensor).c_str(), bid) + "." + suffix,
        tensor);
  }
};
...
// [UMA-FIX] Compute ALL ops in a node range on CPU.
// Used by per-layer execution: after Metal finishes attention, CPU handles
// the sparse FFN phase AND any downstream dense ops that depend on it
// (e.g., the residual ADD: l_out = ffn_result + ffn_inp).
static void ggml_graph_compute_sparse_cpu(std::vector<uint8_t> &buf,
                                           struct ggml_cgraph *gf,
                                           int node_start, int node_end,
                                           int n_threads) {
  int n_nodes = node_end - node_start;
  if (n_nodes <= 0) return;

  // Create a sub-graph view pointing into the original node array
  struct ggml_cgraph sub_gf = {};
  sub_gf.size    = n_nodes;
  sub_gf.n_nodes = n_nodes;
  sub_gf.n_leafs = 0;
  sub_gf.nodes   = gf->nodes + node_start;
  sub_gf.grads   = NULL;
  sub_gf.leafs   = NULL;

  ggml_graph_compute_helper(buf, &sub_gf, n_threads);
}
...
struct llama_hparams {
  ...
  float sparse_pred_threshold =
      (float)atof(getenv("LLAMA_SPARSE_PRED_THRESHOLD")
                      ? getenv("LLAMA_SPARSE_PRED_THRESHOLD")
                      : "0.0");
...
struct llama_layer {
  ...
  struct ggml_tensor
      *gpu_bucket; // double index from GPU split neuron to original neuron
};
...
struct llama_model_loader {
  ...
  struct ggml_tensor *
  create_tensor(struct ggml_context *ctx,
                const std::pair<std::string, llm_tensor> &tn,
                const std::vector<int64_t> &ne, ggml_backend_type backend) {
...
static void llm_load_hparams(llama_model_loader &ml, llama_model &model) {
  ...
  if (gguf_get_sparse_deriv(ctx)) {
    ...
    GGUF_GET_KEY(ctx, hparams.sparse_pred_threshold, gguf_get_val_f32,
                 GGUF_TYPE_FLOAT32, false, kv(LLM_KV_SPARSE_THRESHOLD));
    if (getenv("LLAMA_SPARSE_PRED_THRESHOLD"))
      hparams.sparse_pred_threshold =
          (float)atof(getenv("LLAMA_SPARSE_PRED_THRESHOLD"));
  }
  ...
  case LLM_ARCH_MPT: {
    ...
    GGUF_GET_KEY(ctx, hparams.f_norm_eps, gguf_get_val_f32, GGUF_TYPE_FLOAT32,
                 true, kv(LLM_KV_ATTENTION_LAYERNORM_EPS));
    GGUF_GET_KEY(ctx, hparams.f_clamp_kqv, gguf_get_val_f32, GGUF_TYPE_FLOAT32,
                 false, kv(LLM_KV_ATTENTION_CLAMP_KQV));
    GGUF_GET_KEY(ctx, hparams.f_max_alibi_bias, gguf_get_val_f32,
                 GGUF_TYPE_FLOAT32, true, kv(LLM_KV_ATTENTION_MAX_ALIBI_BIAS));
...
struct llama_gpu_split_loader {
  ...
  llama_gpu_split_loader(const std::string &fname, bool use_mmap)
      : fname(fname), use_mmap(use_mmap) {
    GGML_ASSERT(use_mmap);

    idx_loader = new llama_model_loader(fname, use_mmap);
    GGUF_GET_KEY(idx_loader->ctx_gguf, vram_required, gguf_get_val_u64,
                 GGUF_TYPE_UINT64, true,
                 LLM_KV_NAMES[LLM_KV_SPLIT_VRAM_CAPACITY]);
    ...
    size_t per_tensor_meta_size =
        GGML_PAD(sizeof(struct ggml_tensor), GGML_MEM_ALIGN) + GGML_OBJECT_SIZE;
  ...
  int load_gpu_idx_for_model(llama_model *model) {
    ...
    if (n_tensors != n_layers * 2) {
      LLAMA_LOG_ERROR("%s: error: the number of gpu splits does not match the "
                      "layer of model\n",
                      __func__);
      return 1;
    }
    ...
    for (int il = 0; il < n_layers; il++) {
      ...
      model_layer.gpu_idx =
          idx_loader->create_tensor_for(ctx_meta, gpu_idx, GGML_BACKEND_CPU);
      model_layer.gpu_bucket =
          idx_loader->create_tensor_for(ctx_meta, gpu_bucket, GGML_BACKEND_CPU);
    }
    ...
    for (int il = 0; il < n_layers; il++) {
      ...
      if (gpu_neurons == 0 || gpu_neurons == gpu_idx->ne[0]) {
        ...
        // [UMA-FIX BP7] On Metal UMA, no copy needed — shared memory.
        // Just mark tensor as GPU so the graph builder knows it's GPU-accessible.
        ggml_set_backend(gpu_bucket, GGML_BACKEND_GPU);
#else
        // GGML_ASSERT(false && "cublas is not enabled");
        LLAMA_LOG_WARN("%s: no GPU backend available for gpu_bucket offload\n", __func__);
#endif
      }
    }
...
struct llama_augmentation_model_loader {
  ...
  llama_augmentation_model_loader(llama_model *model) {
    ...
    // const int64_t ggml_aux_tensor_size = 4 * (100 * 100 + 5120*40*4 *
    // ggml_tensor_overhead() + (int64_t)13824*5120*40*4);
    int model_layer = model->layers.size();
    int ffn_dim = model->layers[0].ffn_up->ne[1];
    const size_t ggml_aux_tensor_size =
        4 * (model_layer * ffn_dim * sizeof(float) * 2 +
             model_layer * ffn_dim * sizeof(float) * ggml_tensor_overhead());
  ...
  ggml_tensor *create_striped_mat_to_gpu(struct ggml_tensor *src,
    ...
    ggml_tensor *gpu_dst =
        ggml_new_tensor_2d(aux_ctx, src->type, row_len, gpu_rows);
    ...
    *ggml_cuda_get_data_pp(device_mat_row) = *ggml_cuda_get_data_pp(gpu_dst);
    ...
    const size_t row_data_size =
        ne0 * ggml_type_size(type) / ggml_blck_size(type);
    for (int i = 0; i < gpu_rows; i++) {
      int32_t host_i = ((int32_t *)gpu_bucket->data)[i];
      host_mat_row->data = (char *)(src->data) + host_i * row_data_size;
      char **gpu_data_pp =
          reinterpret_cast<char **>(ggml_cuda_get_data_pp(device_mat_row));
      // printf("gpu_data_p: %p\n", *gpu_data_pp);
    ...
    return gpu_dst;
#elif defined(GGML_USE_METAL)
    // [UMA-FIX BP7] Create striped tensor with only hot neuron rows.
    // On CUDA this copies to separate VRAM. On UMA, we allocate in shared
    // RAM via malloc — GPU can access it directly via auto-registration.
    // We MUST create a smaller tensor (not reuse the full one) because the
    // GPU command buffer working set is limited to ~12GB. Full tensors
    // = 14.7GB (OOM). Striped tensors = ~3.5GB (fits).
    if (gpu_bucket == NULL) {
      // offload the whole tensor — just mark it
      ggml_set_backend(src, GGML_BACKEND_GPU);
      return src;
    }
    ...
    const size_t row_data_size =
        ne0 * ggml_type_size(type) / ggml_blck_size(type);
    const size_t total_size = row_data_size * gpu_rows;

    // Allocate striped tensor metadata in aux_ctx, data via malloc
    ggml_set_no_alloc(aux_ctx, true);
    ggml_tensor *gpu_dst =
        ggml_new_tensor_2d(aux_ctx, src->type, row_len, gpu_rows);
    ggml_set_no_alloc(aux_ctx, false);

    // Allocate page-aligned data so Metal can use newBufferWithBytesNoCopy
    // (zero-copy). Regular malloc is only 16-byte aligned which forces Metal
    // to use newBufferWithBytes (copy), doubling memory usage.
    ...
    int ret = posix_memalign(&aligned_ptr, page_size, alloc_size);
    GGML_ASSERT(ret == 0 && aligned_ptr != NULL && "failed to alloc striped tensor");
    gpu_dst->data = aligned_ptr;
    ggml_set_backend(gpu_dst, GGML_BACKEND_GPU);

    // Copy hot neuron rows into contiguous striped layout
    for (int64_t i = 0; i < gpu_rows; i++) {
      int32_t host_i = ((int32_t *)gpu_bucket->data)[i];
      memcpy((char *)(gpu_dst->data) + i * row_data_size,
             (char *)(src->data) + host_i * row_data_size,
             row_data_size);
    }
  ...
  size_t slice_ffn_mat_to_gpu(llama_layer &layer) {
    ...
    if (layer.ffn_gate) {
      layer.ffn_gate_gpu =
          create_striped_mat_to_gpu(layer.ffn_gate, gpu_bucket);
    ...
    layer.ffn_down_gpu =
        create_striped_mat_to_gpu(layer.ffn_down_t, gpu_bucket);
  ...
  size_t offload_ffn_split(llama_model *model) {
    ...
    ggml_metal_set_sparse_threshold(model->hparams.sparse_pred_threshold);
#endif
    ...
    for (llama_layer &model_layer : model->layers) {
      ...
      if (model_layer.gpu_idx == NULL && model_layer.gpu_bucket == NULL) {
        ggml_tensor *gpu_idx = ggml_new_tensor_1d(
            aux_ctx, GGML_TYPE_I32, model_layer.mlp_pre_w2->ne[1]);
...
struct buffered_tensor_allocator {
  ...
  ggml_context *ctx;
  std::map<tensor_offloading_levels,
           std::vector<std::tuple<int, llm_tensor, ggml_tensor *>>>
      alloc_queues;
  ...
  buffered_tensor_allocator(llama_model_loader &ml, ggml_context *ctx,
                            const llama_hparams &hparams)
      : ml(ml), ctx(ctx), hparams(hparams) {}

  ggml_tensor *buffered_alloc(const std::string &name,
                              const llm_tensor tensor_type,
                              const std::vector<int64_t> &ne,
                              const int i_layer) {
// [UMA-FIX BP4b] Original: only CUBLAS could queue GPU tensors
// #if defined(GGML_USE_CUBLAS)
#if defined(GGML_USE_CUBLAS) || defined(GGML_USE_METAL)
    ...
    ggml_tensor *meta_tensor =
        ml.create_tensor(ctx, name, ne, GGML_BACKEND_CPU);
    ggml_set_no_alloc(ctx, no_alloc);
    alloc_queues[level].push_back(
        std::make_tuple(i_layer, tensor_type, meta_tensor));
    return meta_tensor;
#else
    return ml.create_tensor(ctx, name, ne, GGML_BACKEND_CPU);
#endif
  ...
  int flush() {
// [UMA-FIX BP4c] Original: only CUBLAS could flush GPU tensors
// #if defined(GGML_USE_CUBLAS)
#if defined(GGML_USE_CUBLAS) || defined(GGML_USE_METAL)
#if defined(GGML_USE_CUBLAS)
    if (!ggml_cublas_loaded()) {
    ...
    for (int enum_i = TENSOR_OFFLOAD_ATTN; enum_i <= TENSOR_OFFLOAD_OUTPUT;
         enum_i++) {
      tensor_offloading_levels level =
          static_cast<tensor_offloading_levels>(enum_i);
      for (auto tensor_tup : alloc_queues[level]) {
        ...
        if (level == TENSOR_OFFLOAD_ATTN &&
            tensor_type == LLM_TENSOR_ATTN_OUT) {
          offloaded_layers = i_layer + 1;
        } else if (level == TENSOR_OFFLOAD_OUTPUT) {
          offloaded_layers =
              hparams.n_layer + 1; // indicate all layers + output are offloaded
        }
      }
    }
    ml.done_getting_tensors();
...
#else // GGML_USE_CUBLAS || GGML_USE_METAL
    return 0;
#endif
  }
};

static bool load_gpu_split_from_split_file(llama_model &model,
...
static bool llm_load_gpu_split_with_budget(llama_model_loader &ml,
                                           llama_model &model,
                                           size_t vram_allocatable_bytes,
                                           bool no_cache) {
  ...
  if (access(cached_split_path.c_str(), F_OK) == 0 && !no_cache) {
    if (load_gpu_split_from_split_file(model, cached_split_path,
                                       vram_allocatable_bytes)) {
      return true;
    }
    LLAMA_LOG_ERROR(
        "%s: error: failed to apply previously generated gpu split from '%s'\n",
  ...
  int slice_size = ffn_up->ne[1] * ggml_type_size(ffn_up->type) /
                   ggml_blck_size(ffn_up->type);
  // For model arch with FFN gate, the gate is also sliced, otherwise only the
  // up and down matrices are sliced
  int vram_bytes_per_slice =
      slice_size * (ffn_gate ? 4.5 : 2); // TODO: why 4.5, not 3?
  int neuron_cap =
      floor((double)vram_allocatable_bytes / vram_bytes_per_slice) * 4;

  LLAMA_LOG_INFO("invoking powerinfer Python module to generate gpu split for "
                 "%.2f MiB of VRAM\n",
  ...
  command_ss << "python3 -m powerinfer"
             ...
             << " --capacity " << neuron_cap << " --vram-capacity "
             << vram_allocatable_bytes << " --output " << cached_split_path;
  if (system(command_ss.str().c_str()) != 0 ||
      access(cached_split_path.c_str(), F_OK) != 0) {
    LLAMA_LOG_ERROR("%s: error: failed to generate gpu split\n", __func__);
    return false;
  }

  return load_gpu_split_from_split_file(model, cached_split_path,
                                        vram_allocatable_bytes);
}

static size_t llm_load_gpu_split(llama_model_loader &ml, llama_model &model,
                                 bool no_cache, bool no_offload) {
#if defined(GGML_USE_CUBLAS)
  if (!ggml_cublas_loaded()) {
    throw std::runtime_error(
        format("cannot offload to GPU: " GGML_CUDA_NAME " not loaded"));
  }
#endif

  if (!no_offload &&
      !llm_load_gpu_split_with_budget(ml, model, vram_budget_bytes, no_cache)) {
    LLAMA_LOG_ERROR(
        "%s: error: failed to generate gpu split, an empty one will be used\n",
        __func__);
  }
  ...
  LLAMA_LOG_INFO("%s: offloaded %.2f MiB of FFN weights to GPU\n", __func__,
                 ffn_offloaded_bytes / 1024.0 / 1024.0);

  return ffn_offloaded_bytes;
}

static void
llm_load_sparse_model_tensors(llama_model_loader &ml, llama_model &model,
                              const llama_context_params *cparams, int main_gpu,
                              long int vram_budget_bytes, bool reset_gpu_index,
                              bool disable_ffn_split, bool use_mlock,
                              llama_progress_callback progress_callback,
...
// [UMA-FIX BP1] Added Metal case for sparse model GPU backend selection
#ifdef GGML_USE_CUBLAS
  if (ggml_cublas_loaded()) {
...
#elif defined(GGML_USE_METAL)
  LLAMA_LOG_INFO("%s: using Metal for GPU acceleration (UMA)\n", __func__);
  llama_backend_offload = GGML_BACKEND_GPU;
  llama_backend_offload_split = GGML_BACKEND_GPU;
#endif
...

  {
    ...
    case LLM_ARCH_BAMBOO: {
      model.tok_embd =
          create_tensor(tn(LLM_TENSOR_TOKEN_EMBD, "weight"), {n_embd, n_vocab});

      // output
      {
        model.output_norm =
            create_tensor(tn(LLM_TENSOR_OUTPUT_NORM, "weight"), {n_embd});
        model.output =
            create_tensor(tn(LLM_TENSOR_OUTPUT, "weight"), {n_embd, n_vocab});
      }
      ...
      for (uint32_t &i = current_layer; i < n_layer; ++i) {
        ...
        layer.attn_norm =
            create_tensor(tn(LLM_TENSOR_ATTN_NORM, "weight", i), {n_embd});

        layer.wq =
            create_tensor(tn(LLM_TENSOR_ATTN_Q, "weight", i), {n_embd, n_embd});
        ...
        layer.ffn_norm =
            create_tensor(tn(LLM_TENSOR_FFN_NORM, "weight", i), {n_embd});

        layer.ffn_gate =
            create_tensor(tn(LLM_TENSOR_FFN_GATE, "weight", i), {n_embd, n_ff});
        layer.ffn_down_t = create_tensor(tn(LLM_TENSOR_FFN_DOWN_T, "weight", i),
                                         {n_embd, n_ff});
        layer.mlp_pre_w1 =
            create_tensor(tn(LLM_TENSOR_MLP_PRED_FC1, "weight", i),
                          {n_embd, GGML_NE_WILDCARD});
        layer.mlp_pre_w2 = create_tensor(
            tn(LLM_TENSOR_MLP_PRED_FC2, "weight", i), {GGML_NE_WILDCARD, n_ff});
        layer.ffn_up =
            create_tensor(tn(LLM_TENSOR_FFN_UP, "weight", i), {n_embd, n_ff});
      }
    } break;
    case LLM_ARCH_OPT: {
      model.tok_embd =
          create_tensor(tn(LLM_TENSOR_TOKEN_EMBD, "weight"), {n_embd, n_vocab});
      model.pos_embd = create_tensor(tn(LLM_TENSOR_POS_EMBD, "weight"),
                                     {n_embd, hparams.n_ctx_train});
      // output
      {
        model.output_norm =
            create_tensor(tn(LLM_TENSOR_OUTPUT_NORM, "weight"), {n_embd});
        model.output_norm_b =
            create_tensor(tn(LLM_TENSOR_OUTPUT_NORM, "bias"), {n_embd});
        //    model.output      = create_tensor(tn(LLM_TENSOR_TOKEN_EMBD,
      ...
      for (uint32_t &i = current_layer; i < n_layer; ++i) {
        ...
        layer.attn_norm =
            create_tensor(tn(LLM_TENSOR_ATTN_NORM, "weight", i), {n_embd});
        layer.attn_norm_b =
            create_tensor(tn(LLM_TENSOR_ATTN_NORM, "bias", i), {n_embd});
        ...
        layer.wk = create_tensor(tn(LLM_TENSOR_ATTN_K, "weight", i),
                                 {n_embd, n_embd_gqa});
        layer.bk =
            create_tensor(tn(LLM_TENSOR_ATTN_K, "bias", i), {n_embd_gqa});

        layer.wv = create_tensor(tn(LLM_TENSOR_ATTN_V, "weight", i),
                                 {n_embd, n_embd_gqa});
        layer.bv =
            create_tensor(tn(LLM_TENSOR_ATTN_V, "bias", i), {n_embd_gqa});
        ...
        layer.ffn_norm =
            create_tensor(tn(LLM_TENSOR_FFN_NORM, "weight", i), {n_embd});
        layer.ffn_norm_b =
            create_tensor(tn(LLM_TENSOR_FFN_NORM, "bias", i), {n_embd});

        layer.ffn_down_t = create_tensor(tn(LLM_TENSOR_FFN_DOWN_T, "weight", i),
                                         {n_embd, n_ff});
        layer.ffn_down_b =
            create_tensor(tn(LLM_TENSOR_FFN_DOWN_T, "bias", i), {n_embd});
        ...
        layer.ffn_up =
            create_tensor(tn(LLM_TENSOR_FFN_UP, "weight", i), {n_embd, n_ff});
        layer.ffn_up_b =
            create_tensor(tn(LLM_TENSOR_FFN_UP, "bias", i), {n_ff});
      }
    } break;
    case LLM_ARCH_FALCON: {
      model.tok_embd =
      ...
      for (uint32_t &i = current_layer; i < n_layer; ++i) {
        ...
        if (gguf_find_tensor(
                ml.ctx_gguf,
                tn(LLM_TENSOR_ATTN_NORM_2, "weight", i).first.c_str()) >= 0) {
          layer.attn_norm_2 =
              create_tensor(tn(LLM_TENSOR_ATTN_NORM_2, "weight", i), {n_embd});
          layer.attn_norm_2_b =
              create_tensor(tn(LLM_TENSOR_ATTN_NORM_2, "bias", i), {n_embd});
        }
  ...
  LLAMA_LOG_INFO("%s: offloaded layers from VRAM budget(%ld bytes): %d/%d\n",
                 __func__, vram_budget_bytes, model.n_gpu_layers,
                 hparams.n_layer);
  ...
  model.ffn_offloaded_bytes =
      llm_load_gpu_split(ml, model, reset_gpu_index,
                         disable_ffn_split || !alloc.tensor_offload_complete);
...
void llama_reserve_model_kv_cache(llama_model *model,
...
// [UMA-FIX BP1] Added Metal case for dense model GPU backend selection
#ifdef GGML_USE_CUBLAS
  if (ggml_cublas_loaded()) {
    LLAMA_LOG_INFO("%s: using " GGML_CUDA_NAME " for GPU acceleration\n",
                   __func__);
  ...
  size_t vram_weights = 0;
  {
    ...
    case LLM_ARCH_BAMBOO: {
      ...
      for (uint32_t i = 0; i < n_layer; ++i) {
        ...
        layer.ffn_up = ml.create_tensor(ctx, tn(LLM_TENSOR_FFN_UP, "weight", i),
                                        {n_embd, n_ff}, backend_split);
    ...
    case LLM_ARCH_OPT: {
                           ...
                           {n_embd, hparams.n_ctx_train}, GGML_BACKEND_CPU);
      {
        ...
        // "weight"), {n_embd, n_vocab}, backend_output); // same as token_embed
      ...
      for (uint32_t i = 0; i < n_layer; ++i) {
        ...
        layer.wq = ml.create_tensor(ctx, tn(LLM_TENSOR_ATTN_Q, "weight", i),
                                    {n_embd, n_embd}, backend_split);
        layer.bq = ml.create_tensor(ctx, tn(LLM_TENSOR_ATTN_Q, "bias", i),
                                    {n_embd}, backend_split);

        layer.wk = ml.create_tensor(ctx, tn(LLM_TENSOR_ATTN_K, "weight", i),
                                    {n_embd, n_embd_gqa}, backend_split);
        layer.bk = ml.create_tensor(ctx, tn(LLM_TENSOR_ATTN_K, "bias", i),
                                    {n_embd_gqa}, backend_split);

        layer.wv = ml.create_tensor(ctx, tn(LLM_TENSOR_ATTN_V, "weight", i),
                                    {n_embd, n_embd_gqa}, backend_split);
        layer.bv = ml.create_tensor(ctx, tn(LLM_TENSOR_ATTN_V, "bias", i),
                                    {n_embd_gqa}, backend_split);
        ...
        layer.ffn_down_b = ml.create_tensor(
            ctx, tn(LLM_TENSOR_FFN_DOWN, "bias", i), {n_embd}, backend_split);

        layer.ffn_up = ml.create_tensor(ctx, tn(LLM_TENSOR_FFN_UP, "weight", i),
                                        {n_embd, n_ff}, backend_split);
        layer.ffn_up_b = ml.create_tensor(ctx, tn(LLM_TENSOR_FFN_UP, "bias", i),
                                          {n_ff}, backend_split);
    ...
    case LLM_ARCH_PERSIMMON: {
...

      {
        ...
        if (n_gpu_layers > int(n_layer)) {
          ...
          if (n_gpu_layers > int(n_layer + 1)) {
            LLAMA_LOG_ERROR(
                "%s: CUDA backend missing Persimmon CUDA ops, can offload at "
                "most %ld layers. See: "
                "https://github.com/ggerganov/llama.cpp/issues/4038\n",
                __func__, n_layer + 1);
    ...
    case LLM_ARCH_STABLELM: {
      ...
      for (uint32_t i = 0; i < n_layer; ++i) {
        /*
        llama_model_loader: - tensor    4:         blk.0.attn_output.weight f16
        [  2560,  2560,     1,     1 ]
        */
        const ggml_backend_type backend = int(i) < i_gpu_start
                                              ? GGML_BACKEND_CPU
...
static bool llama_model_load(const std::string &fname, llama_model &model,
  ...
  try {
    ...
    if (ml.sparse_deriv == GGML_SPARSE_INFERENCE) {
      LLAMA_LOG_INFO(
          "%s: PowerInfer model loaded. Sparse inference will be used.\n",
          __func__);
    }
    ...
    if (llama_use_sparse_inference(&model)) {
      if (params.n_gpu_layers > 0) {
        LLAMA_LOG_WARN("%s: sparse inference ignores n_gpu_layers, you can use "
                       "--vram-budget option instead\n",
                       __func__);
        // return false; // Hacked for Powerinfer Metal integration
      }
      llama_set_vram_budget(params.vram_budget_gb, params.main_gpu);
      llm_load_sparse_model_tensors(
          ml, model, cparams, params.main_gpu, vram_budget_bytes,
          params.reset_gpu_index, params.disable_gpu_index, params.use_mlock,
...
using llm_build_cb_short =
    std::function<void(struct ggml_tensor *cur, const char *name)>;
...
enum llm_ffn_gate_type {
  ...
  LLM_FFN_SYM, // ffn_gate is parallel to ffn_up and should pass through an
               // activation function
};
...
llm_build_kv_store(struct ggml_context *ctx, const llama_hparams &hparams,
                   const llama_kv_cache &kv, struct ggml_cgraph *graph,
                   struct ggml_tensor *k_cur, struct ggml_tensor *v_cur,
                   int64_t n_ctx, int32_t n_tokens, int32_t kv_head,
...
llm_build_norm(struct ggml_context *ctx, struct ggml_tensor *cur,
               const llama_hparams &hparams, struct ggml_tensor *mw,
...
static struct ggml_tensor *
llm_build_ffn(struct ggml_context *ctx, struct ggml_tensor *cur,
...
static struct ggml_tensor *llm_build_sparse_mul_mat(
    struct ggml_context *ctx, struct ggml_tensor *up, struct ggml_tensor *inp,
    struct ggml_tensor *idx, struct ggml_tensor *up_gpu,
    struct ggml_tensor *gpu_index, struct ggml_tensor *gpu_bucket,
    const llm_build_cb_short &cb, const char *name, bool full_gpu) {
  ...
  if (up_gpu) {
    ggml_tensor *out_gpu =
        ggml_mul_mat_idx_upscale(ctx, up_gpu, inp, idx, gpu_bucket, out->ne[0]);
    ggml_cuda_assign_buffers_no_alloc(out_gpu);
    cb(out_gpu, (full_name + "_gpu").c_str());
    out = ggml_add(ctx, out, out_gpu);
    // We don't need to assign buffers here, as the output will be passed into
    // Axpy, which in this case, is also a hybrid operation.
...
static struct ggml_tensor *llm_build_sparse_axpy(
    struct ggml_context *ctx, struct ggml_tensor *w_t, struct ggml_tensor *x,
    struct ggml_tensor *sparse_idx, struct ggml_tensor *wt_gpu,
    struct ggml_tensor *gpu_index, struct ggml_tensor *gpu_bucket,
    const llm_build_cb_short &cb, const char *name, bool full_gpu) {
  std::string full_name = "ffn_" + std::string(name) + "_sparse";
  ggml_tensor *out = nullptr;
  ...
  // Pass gpu_index for hot/cold neuron filtering (needed for parallel mode).
  // When gpu_index is NULL (full_gpu mode), all neurons are processed.
  {
...
static struct ggml_tensor *llm_build_ffn_sparse(
    ...
    struct ggml_tensor *gate_b, struct ggml_tensor *down_t,
    struct ggml_tensor *down_b, struct ggml_tensor *pre_w1,
    struct ggml_tensor *pre_w2, struct ggml_tensor *pred_inpl,
    struct ggml_tensor *gpu_index, struct ggml_tensor *gpu_bucket,
    struct ggml_tensor *gate_gpu, struct ggml_tensor *down_gpu,
    struct ggml_tensor *up_gpu, llm_ffn_op_type type_op,
  ...
  // If the FFN layer is not fully offloaded, we need to transfer the sparsity
  // index back to the CPU to avoid synchronization issues.
  ...
  struct ggml_tensor *up_out =
      llm_build_sparse_mul_mat(ctx, up, ffn_input, idx, up_gpu, gpu_index,
                               gpu_bucket, cb_outer, "up", full_gpu);
  if (up_b) {
  ...
  if (gate) {
    ggml_tensor *gate_input =
        (type_gate == LLM_FFN_PAR || type_gate == LLM_FFN_SYM) ? ffn_input
                                                               : up_out;
    gate_out = llm_build_sparse_mul_mat(ctx, gate, gate_input, idx, gate_gpu,
                                        gpu_index, gpu_bucket, cb_outer, "gate",
                                        full_gpu);
    if (gate_b) {
  ...
  cur = llm_build_sparse_axpy(ctx, down_t, cur, idx, down_gpu, gpu_index,
                              gpu_bucket, cb_outer, "down", full_gpu);
...
static struct ggml_tensor *
llm_build_kqv(struct ggml_context *ctx, const llama_hparams &hparams,
              const llama_kv_cache &kv, struct ggml_tensor *wo,
              struct ggml_tensor *wo_b, struct ggml_tensor *q_cur,
              struct ggml_tensor *kq_scale, struct ggml_tensor *kq_mask,
              int64_t n_ctx, int32_t n_tokens, int32_t n_kv,
...
const llm_build_cb no_offload_cb = [](struct ggml_tensor *cur, const char *name,
                                      int nl) { ggml_set_name(cur, name); };
...
struct llm_build_context {
  ...
  llm_build_context(llama_context &lctx, const llama_batch &batch,
      ...
      : model(lctx.model), hparams(model.hparams), cparams(lctx.cparams),
        batch(batch), kv_self(lctx.kv_self), n_embd(hparams.n_embd),
        n_layer(hparams.n_layer), n_ctx(cparams.n_ctx), n_head(hparams.n_head),
        n_head_kv(hparams.n_head_kv), n_embd_head(hparams.n_embd_head()),
        n_embd_gqa(hparams.n_embd_gqa()), freq_base(cparams.rope_freq_base),
        freq_scale(cparams.rope_freq_scale),
        ext_factor(cparams.yarn_ext_factor),
        attn_factor(cparams.yarn_attn_factor),
        beta_fast(cparams.yarn_beta_fast), beta_slow(cparams.yarn_beta_slow),
        norm_eps(hparams.f_norm_eps), norm_rms_eps(hparams.f_norm_rms_eps),
        n_tokens(batch.n_tokens), n_kv(worst_case ? n_ctx : kv_self.n),
        kv_head(worst_case ? n_ctx - n_tokens : kv_self.head),
        n_orig_ctx(cparams.n_yarn_orig_ctx),
        do_rope_shift(worst_case || kv_self.has_shift), cb(cb),
  ...
  struct ggml_cgraph *build_llama_variants() {
    struct ggml_cgraph *gf =
        ggml_new_graph_custom(ctx0, LLAMA_MAX_NODES, false);
    ...
    for (int il = 0; il < n_layer; ++il) {
      ...
      // feed-forward network
      {
        cur = llm_build_norm(ctx0, ffn_inp, hparams, model.layers[il].ffn_norm,
                             NULL, LLM_NORM_RMS, cb, il);
        llm_ffn_gate_type gate_type =
            model.arch == LLM_ARCH_BAMBOO ? LLM_FFN_SYM : LLM_FFN_PAR;
        ...
        if (llama_use_sparse_inference(&model)) {
          llm_build_cb_short cbs = [&](ggml_tensor *cur, const char *name) {
            ...
            cur->layer_id = il;  // [UMA-FIX] Tag sparse FFN tensors with layer index
          };
          ...
          cur = llm_build_ffn_sparse(
              ctx0, cur, model.layers[il].ffn_up, NULL,
              model.layers[il].ffn_gate, NULL, model.layers[il].ffn_down_t,
              NULL, model.layers[il].mlp_pre_w1, model.layers[il].mlp_pre_w2,
              ...
              model.layers[il].ffn_up_gpu, LLM_FFN_RELU, gate_type,
              model.layers[il].gpu_offload_ratio, cbs);
        } else {
          // fallback to dense
          cb(cur, "ffn_norm", il);
          llm_ffn_op_type act_type =
              model.arch == LLM_ARCH_BAMBOO ? LLM_FFN_RELU : LLM_FFN_SILU;
          cur = llm_build_ffn(ctx0, cur, model.layers[il].ffn_up, NULL,
                              model.layers[il].ffn_gate, NULL,
                              model.layers[il].ffn_down, NULL, act_type,
                              gate_type, cb, il);
        }
      }
    ...
    for (int il = 0; il < n_layer; ++il) {
      ...
      // feed-forward network
      {
        ...
        if (llama_use_sparse_inference(&model)) {
          ...
          cur = llm_build_ffn_sparse(
              ctx0, cur, model.layers[il].ffn_up, model.layers[il].ffn_up_b,
              NULL, NULL, model.layers[il].ffn_down_t,
              model.layers[il].ffn_down_b, model.layers[il].mlp_pre_w1,
              model.layers[il].mlp_pre_w2, ffn_inp, model.layers[il].gpu_idx,
              model.layers[il].gpu_bucket, model.layers[il].ffn_gate_gpu,
              model.layers[il].ffn_down_gpu, model.layers[il].ffn_up_gpu,
              LLM_FFN_RELU, LLM_FFN_SEQ, model.layers[il].gpu_offload_ratio,
              cbs);
        } else {
          cb(cur, "ffn_norm", il);
          cur = llm_build_ffn(
              ctx0, cur, model.layers[il].ffn_up, model.layers[il].ffn_up_b,
              NULL, NULL, model.layers[il].ffn_down,
              model.layers[il].ffn_down_b, LLM_FFN_RELU, LLM_FFN_SEQ, cb, il);
          cb(cur, "ffn_out", il);
        }
      }
  ...
  struct ggml_cgraph *build_falcon() {
    ...
    for (int il = 0; il < n_layer; ++il) {
      ...
      if (llama_use_sparse_inference(&model)) {
        ...
        cur = llm_build_ffn_sparse(
            ctx0, attn_norm, model.layers[il].ffn_up, NULL, NULL, NULL,
            model.layers[il].ffn_down_t, NULL, model.layers[il].mlp_pre_w1,
            model.layers[il].mlp_pre_w2,
            inpL, // Falcon uses the layer's input as the pred input
            model.layers[il].gpu_idx, model.layers[il].gpu_bucket,
            model.layers[il].ffn_gate_gpu, model.layers[il].ffn_down_gpu,
            model.layers[il].ffn_up_gpu, LLM_FFN_RELU, LLM_FFN_SEQ,
            model.layers[il].gpu_offload_ratio, cbs);
      } else {
        cb(attn_norm, "attn_norm", il);
        cur = llm_build_ffn(ctx0,
                            attn_norm, // !! use the attn norm, not the result
                            model.layers[il].ffn_up, NULL, NULL, NULL,
                            model.layers[il].ffn_down_t, NULL, LLM_FFN_RELU,
                            LLM_FFN_SEQ, cb, il);
        cb(cur, "ffn_out", il);
      }
    ...
    for (int il = 0; il < n_layer; ++il) {
      ...
      // self attention
      {
        ...
        struct ggml_tensor *Q =
            ggml_cont(ctx0, ggml_permute(ctx0, Qcur, 2, 1, 0, 3));
        cb(Q, "Q", il);
  ...
  struct ggml_cgraph *build_stablelm() {
    ...
    if (do_rope_shift) {
      llm_build_k_shift(ctx0, hparams, cparams, kv_self, gf, LLM_ROPE_NEOX,
                        n_ctx, hparams.n_rot, freq_base, freq_scale, cb);
    }
    ...
    for (int il = 0; il < n_layer; ++il) {
      ...
      // self-attention
      {
        ...
        // RoPE the first n_rot of q/k, pass the other half, and concat.
        struct ggml_tensor *qrot = ggml_cont(
            ctx0,
            ggml_view_3d(ctx0, tmpq, hparams.n_rot, n_head, n_tokens,
                         ggml_element_size(tmpq) * n_embd_head,
                         ggml_element_size(tmpq) * n_embd_head * n_head, 0));
        cb(qrot, "qrot", il);

        struct ggml_tensor *krot = ggml_cont(
            ctx0,
            ggml_view_3d(ctx0, tmpk, hparams.n_rot, n_head, n_tokens,
                         ggml_element_size(tmpk) * n_embd_head,
                         ggml_element_size(tmpk) * n_embd_head * n_head_kv, 0));
        cb(krot, "krot", il);

        // get the second half of tmpq, e.g tmpq[n_rot:, :, :]
        struct ggml_tensor *qpass =
            ggml_view_3d(ctx0, tmpq, (n_embd_head - hparams.n_rot), n_head,
                         n_tokens, ggml_element_size(tmpq) * n_embd_head,
                         ggml_element_size(tmpq) * n_embd_head * n_head,
                         ggml_element_size(tmpq) * hparams.n_rot);
        cb(qpass, "qpass", il);

        struct ggml_tensor *kpass =
            ggml_view_3d(ctx0, tmpk, (n_embd_head - hparams.n_rot), n_head_kv,
                         n_tokens, ggml_element_size(tmpk) * (n_embd_head),
                         ggml_element_size(tmpk) * (n_embd_head)*n_head_kv,
                         ggml_element_size(tmpk) * hparams.n_rot);
        cb(kpass, "kpass", il);

        struct ggml_tensor *qrotated = ggml_rope_custom(
            ctx0, qrot, inp_pos, hparams.n_rot, 2, 0, n_orig_ctx, freq_base,
            freq_scale, ext_factor, attn_factor, beta_fast, beta_slow);
        cb(qrotated, "qrotated", il);

        struct ggml_tensor *krotated = ggml_rope_custom(
            ctx0, krot, inp_pos, hparams.n_rot, 2, 0, n_orig_ctx, freq_base,
            freq_scale, ext_factor, attn_factor, beta_fast, beta_slow);
        cb(krotated, "krotated", il);
        ...
        cur = llm_build_kqv(ctx0, hparams, kv_self, model.layers[il].wo, NULL,
                            Q, KQ_scale, KQ_mask, n_ctx, n_tokens, n_kv, -1.0f,
                            cb, il);
        cb(cur, "kqv_out", il);
      }
...
// TODO: will be removed with backend v2
static const std::unordered_map<const char *, llm_offload_func_e>
    k_offload_map = {
...
static struct ggml_cgraph *llama_build_graph(llama_context &lctx,
  ...
  llm_build_cb cb = [&](struct ggml_tensor *cur, const char *name, int il) {
    // [UMA-FIX] Tag tensor with layer index for per-layer Metal execution
    cur->layer_id = il;
...
static int llama_decode_internal(llama_context &lctx, llama_batch batch) {
  ...
  const bool full_offload_supported =
      ...
      model.arch == LLM_ARCH_MPT || model.arch == LLM_ARCH_STARCODER ||
      model.arch == LLM_ARCH_STABLELM || model.arch == LLM_ARCH_BAMBOO;

  // const bool fully_offloaded = model.n_gpu_layers >= (int) hparams.n_layer +
  // 3; if (ggml_cpu_has_cublas() && full_offload_supported && fully_offloaded)
  // {
  //     n_threads = 8;
  // }
  ...
  if (lctx.ctx_metal && llama_use_sparse_inference(&model) && !uma_force_cpu) {
    ...
    ggml_metal_set_n_cb(lctx.ctx_metal, 1);

    // Propagate layer_id through the graph: intermediate nodes created by
    // ggml ops (ggml_mul_mat, ggml_add, etc.) don't go through the cb callback
    // and keep the default layer_id=-1. Fix by inheriting from source tensors.
    for (int i = 0; i < gf->n_nodes; i++) {
      struct ggml_tensor *node = gf->nodes[i];
      if (node->layer_id >= 0) continue;
      for (int s = 0; s < GGML_MAX_SRC; s++) {
        if (node->src[s] && node->src[s]->layer_id >= 0) {
          node->layer_id = node->src[s]->layer_id;
          break;
        }
      }
    }

    // Build layer ranges by scanning node layer_ids
    struct layer_range { int start; int end; int layer_id; };
    std::vector<layer_range> ranges;

    int range_start = 0;
    int cur_layer = gf->nodes[0]->layer_id;
    for (int i = 1; i <= gf->n_nodes; i++) {
      int next_layer = (i < gf->n_nodes) ? gf->nodes[i]->layer_id : -999;
      if (next_layer != cur_layer) {
        ranges.push_back({range_start, i, cur_layer});
        range_start = i;
    ...
    for (const auto &r : ranges) {
      // Find the first sparse op in this range
      int first_sparse = r.end;  // default: no sparse ops
      for (int i = r.start; i < r.end; i++) {
        enum ggml_op op = gf->nodes[i]->op;
        if (op == GGML_OP_MUL_MAT_SPARSE || op == GGML_OP_AXPY) {
          first_sparse = i;
          break;
        }
      }
      ...
      if (uma_full_parallel && first_sparse < r.end) {
        ...
        // Three phases:
        //   1. Parallel: GPU+CPU process up to and including AXPY
        //      (Metal AXPY writes to temp, CPU AXPY writes to dst)
        //   2. Merge: dst += temp (combines hot + cold neuron results)
        //   3. Sequential: CPU processes post-AXPY nodes (ADD/l_out)
        //      that depend on the merged AXPY output
        // ============================================================

        // Allocate temp buffer once (ne00 = output dim, typically 4096)
        static bool parallel_temp_allocated = false;
        int last_axpy = -1;
        for (int i = first_sparse; i < r.end; i++) {
        ...
        // Phase 3: CPU processes remaining nodes (ADD/l_out) after merge
        if (parallel_end < r.end) {
          ggml_graph_compute_sparse_cpu(lctx.work_buffer, gf,
                                         parallel_end, r.end, n_threads);
        }
      } else if (first_sparse < r.end) {
        // ============================================================
...
struct fragment_buffer_variant {
  ...
  fragment_buffer_variant(const std::string &_raw_text, int64_t _offset,
                          int64_t _length)
      : type(FRAGMENT_BUFFER_VARIANT_TYPE_RAW_TEXT), token((llama_vocab::id)-1),
        raw_text(_raw_text), offset(_offset), length(_length) {
...
static ggml_type get_k_quant_type(quantize_state_internal &qs,
  ...
  if (name == tn(LLM_TENSOR_OUTPUT, "weight").first) {
    ...
    if (ftype == LLAMA_FTYPE_MOSTLY_Q2_K)
      new_type = GGML_TYPE_Q3_K;
  } else if (name.find("fc1.weight") != std::string::npos ||
             name.find("fc2.weight") != std::string::npos) {
  ...
  if (new_type == GGML_TYPE_Q2_K || new_type == GGML_TYPE_Q3_K ||
    ...
    int ny = tensor->ne[1];
    if (nx % QK_K != 0) {
      LLAMA_LOG_WARN("\n\n%s : tensor cols %d x %d are not divisible by %d, "
                     "required for %s",
...
llama_model_quantize_internal(const std::string &fname_inp,
  ...
  struct gguf_context *ctx_out = ml.sparse_deriv == GGML_SPARSE_INFERENCE
                                     ? gguf_init_empty_sparse()
                                     : gguf_init_empty();
  ...
  if (qs.n_fallback > 0) {
    LLAMA_LOG_WARN("%s: WARNING: %d of %d tensor(s) incompatible with k-quants "
                   "and required fallback quantization\n",
...
struct llama_model *
llama_load_model_from_file_with_context(const char *path_model,
...
llama_new_context_with_model(struct llama_model *model,
  ...
  if (!hparams.vocab_only) {
...

    {
      ...
      ctx->buf_compute.resize(ggml_tensor_overhead() * LLAMA_MAX_NODES +
                              ggml_graph_overhead());
...
#define LLAMA_METAL_CHECK_BUF(result)                                          \
  if (!(result)) {                                                             \
    ...
    return NULL;                                                               \
      ...
      // [UMA-FIX BP6] Skip full data registration for large sparse models.
      // Individual tensors auto-register on demand with proper page alignment.
      const size_t kv_alloc_overhead = ctx->kv_self.buf.size + ctx->buf_alloc.size;
      if (data_size + kv_alloc_overhead > 10ULL * 1024 * 1024 * 1024) {
          LLAMA_LOG_INFO("%s: model data (%.2f GB) exceeds safe limit, "
                         "using per-tensor UMA auto-registration\n",
                         __func__, data_size / 1024.0 / 1024.0 / 1024.0);
      } else {
...
size_t llama_model_offload_ffn_split(struct llama_model *model) {
  llama_augmentation_model_loader *aug_ml =
      new llama_augmentation_model_loader(model);
...
void llama_print_timings(struct llama_context *ctx) {
  ...
  LLAMA_LOG_INFO("%s:        load time = %10.2f ms\n", __func__,
                 timings.t_load_ms);
  LLAMA_LOG_INFO("%s:      sample time = %10.2f ms / %5d runs   (%8.2f ms per "
  ...
  LLAMA_LOG_INFO("%s: prompt eval time = %10.2f ms / %5d tokens (%8.2f ms per "
                 "token, %8.2f tokens per second)\n",
  ...
  LLAMA_LOG_INFO("%s:        eval time = %10.2f ms / %5d runs   (%8.2f ms per "
                 "token, %8.2f tokens per second)\n",
